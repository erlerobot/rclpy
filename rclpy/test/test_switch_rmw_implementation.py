...
def f_rmw(result):
    import rclpy
    if len(rclpy.rmw_implementations) < 2:
        # Ignore this test, we need at least two implementations
        print('Need more than one rmw implementation installed')
        return

    for second_rmw in rclpy.rmw_implementations[1:]:
        try:
            rclpy_impl = importlib.import_module(
    ...
    orig_rmw = rclpy._rclpy.rclpy_get_implementation_identifier()

    rclpy.set_rmw_implementation(rclpy_impl)

    rclpy.init([])

    set_rmw = rclpy._rclpy.rclpy_get_implementation_identifier()
    impl_rmw = rclpy_impl.rclpy_get_implementation_identifier()

    result[0] = (set_rmw == impl_rmw)
